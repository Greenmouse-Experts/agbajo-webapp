...
export function StatCard({ icon: Icon, iconClass, value, label, trend, link }: StatCardProps) {
  return (
    <div className="card bg-base-100 shadow-sm border border-base-200">
      <div className="card-body p-5 gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-base-content/60 mt-0.5">{label}</p>
        </div>
        {link && (
          <Link
            to={link.to as any}
            className={`text-xs hover:underline -mt-1 ${link.colorClass}`}
          >
            {link.label} →
          </Link>
        )}
